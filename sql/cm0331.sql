...
INSERT INTO dev_realtime_test_chenming.ods_product_sku PARTITION(ds='2023-10-01')
SELECT
...
FROM (
         ...
         FROM dev_realtime_test_chenming.ods_product_base
         WHERE ds='2023-10-01'
     ) sku_gen;
...
INSERT INTO dev_realtime_test_chenming.ods_order_detail PARTITION(ds='2023-10-01')
SELECT
...
FROM (
         ...
         FROM dev_realtime_test_chenming.ods_product_base p
                  JOIN dev_realtime_test_chenming.ods_product_sku s ON p.product_id = s.product_id
...
CREATE external TABLE IF NOT EXISTS dev_realtime_test_chenming.ods_user_behavior_log (
  ...
  user_id        STRING COMMENT '用户ID',
  product_id     STRING COMMENT '商品ID',
  behavior_type  STRING COMMENT '行为类型（click/view/cart/pay）', --click（点击）、view（浏览）、cart（加入购物车）和pay（支付）。
...
INSERT  INTO dev_realtime_test_chenming.ods_user_behavior_log PARTITION(ds='2023-10-01')
SELECT
...
INSERT INTO dev_realtime_test_chenming.ods_price_change_log PARTITION(ds='2023-10-01')
SELECT
    p.product_id,
...
tblproperties ("orc.compress"="snappy");
INSERT INTO dev_realtime_test_chenming.ods_product_review PARTITION(ds='2023-10-01')
SELECT
...
INSERT INTO dev_realtime_test_chenming.ods_operation_activity PARTITION(ds='2023-10-01')
SELECT
    activity_id,
    product_id,
    action_type,
...
-- 商品销售核心指标表（ADS层）
CREATE external TABLE IF NOT EXISTS dev_realtime_test_chenming.ads_product_sales_core (
  product_id       STRING    COMMENT '商品ID',
  product_name     STRING    COMMENT '商品名称',
  category_id      STRING    COMMENT '类目ID',
...
--click（点击）、view（浏览）、cart（加入购物车）和pay（支付）。
insert overwrite table dev_realtime_test_chenming.ads_product_sales_core
SELECT

    p.product_id,
    p.product_name,
    p.category_id,
    COUNT(DISTINCT o.order_id) AS order_count, -- 订单计数    反映商品市场接受度
    SUM(o.quantity) AS sales_volume, -- 统计购买数量 体现商品实际流通效率
    SUM(o.amount) AS gmv, -- 总金额 最直观的商业价值体现
    COUNT(DISTINCT o.user_id) AS buyer_count,-- 统计用户 衡量用户覆盖范围
    SUM(o.amount) / COUNT(DISTINCT o.order_id) AS avg_order_value,-- 平均实际支付金额 客单价（avg_order_value）= GMV / 订单量 → 反映用户消费能力
    COUNT(DISTINCT CASE WHEN u.behavior_type = 'click' THEN u.log_id END) AS click_count,-- 点击量
    COUNT(DISTINCT o.order_id) / COUNT(DISTINCT CASE WHEN u.behavior_type = 'click' THEN u.log_id END) AS conversion_rate,-- 转化率 转化率（conversion_rate）= 订单量 / 点击量 → 评估流量质量
    o.ds
FROM
    dev_realtime_test_chenming.ods_product_base p
        LEFT JOIN
    dev_realtime_test_chenming.ods_order_detail o ON p.product_id = o.product_id AND o.ds = '2023-10-01'
        LEFT JOIN
    dev_realtime_test_chenming.ods_user_behavior_log u ON p.product_id = u.product_id AND u.ds = '2023-10-01'
WHERE
...
-- 按天销售趋势
drop  table dev_realtime_test_chenming.ads_product_sales_trend;
CREATE TABLE IF NOT EXISTS dev_realtime_test_chenming.ads_product_sales_trend (
                                                                                  product_id         STRING    COMMENT '商品ID',
...
insert overwrite table dev_realtime_test_chenming.ads_product_sales_trend
SELECT

    p.product_id,
    date_format(o.event_time, 'yyyy-MM-dd') AS sale_date, -- 格式化时间 但是没有模拟出日期
    COUNT(DISTINCT o.order_id) AS daily_order_count, -- 订单id 去重统计
    SUM(o.amount) AS daily_gmv, -- 总金额
    o.ds
FROM
    dev_realtime_test_chenming.ods_product_base p
        JOIN
    dev_realtime_test_chenming.ods_order_detail o ON p.product_id = o.product_id
WHERE
        p.ds = '2023-10-01' AND
...
-- SKU销售分析表（ADS层）
drop  table dev_realtime_test_chenming.ads_sku_sales_analysis;
CREATE TABLE IF NOT EXISTS dev_realtime_test_chenming.ads_sku_sales_analysis (
                                                                                 sku_id          STRING    COMMENT 'SKU唯一标识',
...
insert overwrite table dev_realtime_test_chenming.ads_sku_sales_analysis
SELECT
    ...
    SUM(o.amount) / SUM(SUM(o.amount)) OVER (PARTITION BY p.product_id) AS sales_ratio, --销售占比（占该商品总销售额比例） 销售占比（sales_ratio）= 单 SKU 销售额 / 商品总销售额
        o.ds
FROM
    dev_realtime_test_chenming.ods_product_sku s
        JOIN
    dev_realtime_test_chenming.ods_product_base p ON s.product_id = p.product_id AND p.ds = '2023-10-01'
        LEFT JOIN
    dev_realtime_test_chenming.ods_order_detail o ON s.sku_id = o.sku_id AND o.ds = '2023-10-01'
WHERE
...
-- 价格带分析
drop table dev_realtime_test_chenming.ads_product_price_band_analysis;
CREATE TABLE IF NOT EXISTS dev_realtime_test_chenming.ads_product_price_band_analysis (
                                                                                          product_id          STRING    COMMENT '商品ID',
                                                                                          product_name        STRING    COMMENT '商品名称',
                                                                                          price              DOUBLE    COMMENT '商品价格',
...
INSERT OVERWRITE TABLE dev_realtime_test_chenming.ads_product_price_band_analysis
SELECT
    p.product_id,
    p.product_name,
    p.price,
    CASE
        ...
        END AS price_band,
    COUNT(DISTINCT o.order_id) AS order_count,
    SUM(o.amount) AS gmv, --调价前后销售对比（GMV / 销量变化）
    AVG(o.amount / NULLIF(o.quantity, 0)) AS avg_selling_price,  -- 添加NULLIF避免除零错误 两个参数相等时返回 NULL，若不相等则返回第一个参数 实际平均售价
    o.ds
FROM
    dev_realtime_test_chenming.ods_product_base p
        LEFT JOIN
...
-- 价格变动影响分析
drop  table dev_realtime_test_chenming.ads_price_change_impact;
CREATE TABLE IF NOT EXISTS dev_realtime_test_chenming.ads_price_change_impact (
                                                                                  product_id          STRING   COMMENT '商品ID',
                                                                                  product_name        STRING   COMMENT '商品名称',
...
insert overwrite  table dev_realtime_test_chenming.ads_price_change_impact
SELECT
    p.product_id,
    p.product_name,
    pc.old_price,-- 变动前价格
    pc.new_price,-- 变动后价格
    (pc.new_price - pc.old_price) / pc.old_price AS price_change_rate, -- 价格变动率
    pc.ds
FROM
    dev_realtime_test_chenming.ods_price_change_log pc
        JOIN
    dev_realtime_test_chenming.ods_product_base p ON pc.product_id = p.product_id AND p.ds = '2023-10-01'
        LEFT JOIN
    dev_realtime_test_chenming.ods_order_detail o ON pc.product_id = o.product_id AND o.ds = '2023-10-01'
WHERE
...
-- 流量渠道分析
drop table dev_realtime_test_chenming.ads_product_channel_performance;
CREATE TABLE IF NOT EXISTS dev_realtime_test_chenming.ads_product_channel_performance (
                                                                                          product_id       STRING    COMMENT '商品ID',
                                                                                          product_name     STRING    COMMENT '商品名称',
                                                                                          channel         STRING    COMMENT '流量渠道（搜索/推荐/直播）',
...
insert overwrite table dev_realtime_test_chenming.ads_product_channel_performance
SELECT
    p.product_id,
    p.product_name,
    u.channel,
    COUNT(DISTINCT u.log_id) AS pv,  --各渠道 UV/PV 分布（搜索 / 推荐 / 直播
    COUNT(DISTINCT u.user_id) AS uv, --渠道转化率对比
    u.ds
FROM
    dev_realtime_test_chenming.ods_product_base p
        LEFT JOIN
...
--标题优化
CREATE external TABLE IF NOT EXISTS dev_realtime_test_chenming.ads_product_title_optimization (
  product_id         STRING    COMMENT '商品ID',
  product_name       STRING    COMMENT '商品名称',
...
-- 数据生成逻辑（需根据实际分词逻辑调整）
insert overwrite table dev_realtime_test_chenming.ads_product_title_optimization
SELECT
    p.product_id,
    p.product_name,
    ...
    CASE  -- 核心词根引流效果
        ...
        WHEN regexp_extract(p.product_name, '.*(加厚|新款|冬季).*', 1) IS NOT NULL THEN '修饰词'  -- 搜索词类型分布（品类词 / 修饰词 / 长尾词）
        ELSE '长尾词'
...
FROM
    dev_realtime_test_chenming.ods_product_base p
        LEFT JOIN
    dev_realtime_test_chenming.ods_user_behavior_log u
    ON p.product_id = u.product_id
        AND u.ds = '2023-10-01'
        AND u.behavior_type = 'click'
        LEFT JOIN
    dev_realtime_test_chenming.ods_order_detail o
...
-- 商品关联分析
drop table if exists dev_realtime_test_chenming.ads_product_behavior_analysis;
CREATE TABLE IF NOT EXISTS dev_realtime_test_chenming.ads_product_behavior_analysis (
                                                                                        product_id       STRING    COMMENT '商品ID',
                                                                                        product_name     STRING    COMMENT '商品名称',
...
insert  overwrite table dev_realtime_test_chenming.ads_product_behavior_analysis
SELECT
    p.product_id,
    p.product_name,
    u.behavior_type,
...
-- 评价分析\
drop table dev_realtime_test_chenming.ads_product_review;
CREATE TABLE IF NOT EXISTS dev_realtime_test_chenming.ads_product_review (
...
insert overwrite  table dev_realtime_test_chenming.ads_product_review --商品健康度
SELECT
    r.product_id,
    p.product_name,
    AVG(r.score) AS avg_score, --平均评分
    ...
    r.ds
FROM
    dev_realtime_test_chenming.ods_product_review r
        JOIN
    dev_realtime_test_chenming.ods_product_base p ON r.product_id = p.product_id AND p.ds = '2023-10-01'
WHERE
        r.ds = '2023-10-01'
GROUP BY
...
-- 删除表
drop table if exists dev_realtime_test_chenming.dwt_product_360_view;

-- 创建商品360度综合视图表（dwt）
CREATE EXTERNAL TABLE IF NOT EXISTS dev_realtime_test_chenming.dwt_product_360_view (
    product_id STRING COMMENT '商品ID',
    product_name STRING COMMENT '商品名称',
    category_id STRING COMMENT '类目ID',
    price DOUBLE COMMENT '商品价格',
...
WITH combined_data AS (
    SELECT
        ...
        base.ds
    FROM
        dev_realtime_test_chenming.ods_product_base base
            -- 连接销售核心指标表
            LEFT JOIN dev_realtime_test_chenming.ads_product_sales_core sales_core
                      ON base.product_id = sales_core.product_id AND base.ds = sales_core.ds
            -- 连接SKU销售分析表
            LEFT JOIN dev_realtime_test_chenming.ads_sku_sales_analysis sku
                      ON base.product_id = sku.product_id AND base.ds = sku.ds
            -- 连接价格带分析表
            LEFT JOIN dev_realtime_test_chenming.ads_product_price_band_analysis price_band
                      ON base.product_id = price_band.product_id AND base.ds = price_band.ds
            -- 连接价格变动影响分析表
            LEFT JOIN dev_realtime_test_chenming.ads_price_change_impact price_change
                      ON base.product_id = price_change.product_id AND base.ds = price_change.ds
            -- 连接流量渠道分析表
            LEFT JOIN dev_realtime_test_chenming.ads_product_channel_performance channel_performance
                      ON base.product_id = channel_performance.product_id AND base.ds = channel_performance.ds
            -- 连接标题优化分析表
            LEFT JOIN dev_realtime_test_chenming.ads_product_title_optimization title_optimization
                      ON base.product_id = title_optimization.product_id AND base.ds = title_optimization.ds
            -- 连接商品用户行为分析表
            LEFT JOIN dev_realtime_test_chenming.ads_product_behavior_analysis behavior_analysis
                      ON base.product_id = behavior_analysis.product_id AND base.ds = behavior_analysis.ds
            -- 连接商品评价分析表
            LEFT JOIN dev_realtime_test_chenming.ads_product_review review
                      ON base.product_id = review.product_id AND base.ds = review.ds
            -- 连接运营活动表
            LEFT JOIN dev_realtime_test_chenming.ods_operation_activity activity
                      ON base.product_id = activity.product_id AND base.ds = activity.ds
                          AND activity.end_time > CURRENT_TIMESTAMP
    WHERE
            base.ds = '2023-10-01'
)
INSERT OVERWRITE TABLE dev_realtime_test_chenming.dwt_product_360_view
SELECT
    product_id,
    product_name,
    category_id,
